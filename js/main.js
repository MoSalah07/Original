...
document.querySelector("#chating").addEventListener("keyup", (e) => {
  if (e.key === "Enter") {
    ...
    e.target.value = "";
    let filterdArr = users.filter((elem) => {
      return elem.id !== foundName.id;
    });
    filterdArr = [foundName, ...filterdArr];
    setLocalStorage(filterdArr);
  }
  sendMessages(foundName.messages);
});
...
sendMessages(users[0].messages);
document.querySelector(".title-user").textContent = users[0].name;
